...
export default function VistaPedidosAdmin() {
    ...
    const getAllOrders = async () => {
        ...
        (await getOrders()).map(async (order) => {
            ...
            (await findOrderProductById(order.id)).map(async (op) => {
                //guardamos el producto
                setProduct(await findProductById(op.id_product));
                productos.concat({
                    name: (await findProductById(op.id_product))!.name,
                    quantity: op.quantity,
                    price: (await findProductById(op.id_product))!.price
                })
            })
